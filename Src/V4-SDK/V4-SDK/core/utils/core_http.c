...
static int32_t _core_http_recv_header(core_http_handle_t *http_handle, uint32_t *body_total_len)
{
    ...
    header = http_handle->sysdep->core_sysdep_malloc(CORE_HTTP_HEAD_ESTIMATED_LEN, CORE_HTTP_MODULE_NAME);
    if (header == NULL) {
        return STATE_SYS_DEPEND_MALLOC_FAILED;
    }
    memset(header, 0, CORE_HTTP_HEAD_ESTIMATED_LEN);
    int pre_len = 0;
    int cnt = 0;

    timenow_ms = http_handle->sysdep->core_sysdep_time();
    for (cnt = 0; cnt < CORE_HTTP_HEAD_RETRY_MAX; cnt++) {
        ...
        /* read http header, "\r\n" in the end */
        if ((res = _core_http_recv(http_handle, (uint8_t *)(header + pre_len), CORE_HTTP_HEAD_ESTIMATED_LEN,
        ...
        if ((NULL == (end = strstr(header, "\r\n\r\n")))) {
            continue;
        }

        char *CONTENT_LEN = "Content-Length";
        char *tmp_ptr = strstr(header, "Content-Length: ");
        if (NULL == tmp_ptr) {
            break;
        }

        char *content_len_str_start = tmp_ptr + strlen("Content-Length: ");
        char *content_len_str_end = strstr(tmp_ptr, "\r\n");
        int32_t input_len = content_len_str_end - content_len_str_start;

        char content_len_str[50] = {0};
        memcpy(content_len_str, content_len_str_start, content_len_str_end - content_len_str_start);
        core_str2uint(content_len_str, input_len, body_total_len);
				core_log1(http_handle->sysdep, STATE_HTTP_BASE, "content len is %s\n", content_len_str);
        int header_len = end - header + strlen("\r\n\r\n");
        int remain_body_len = pre_len - header_len;
        _core_http_recv_header_pair(http_handle, CONTENT_LEN, strlen(CONTENT_LEN), content_len_str, input_len);

        uint32_t status_code = 206;
        _core_http_recv_status_code(http_handle, status_code);
        char *body_start = header + header_len;

        if (remain_body_len> 0) {
            aiot_http_recv_t packet;

            //core_log_hexdump(STATE_HTTP_LOG_RECV_CONTENT, '<', (uint8_t *)body_start, remain_body_len);
            if (http_handle->core_recv_handler != NULL) {
                http_handle->session.body_read_len += remain_body_len ;
                memset(&packet, 0, sizeof(aiot_http_recv_t));
                packet.type = AIOT_HTTPRECV_BODY;
                packet.data.body.buffer = (uint8_t *)body_start;
                packet.data.body.len = remain_body_len;
...
static int32_t _core_http_recv_body(core_http_handle_t *http_handle, uint32_t body_total_len)
{
    ...
    http_handle->sysdep->core_sysdep_mutex_lock(http_handle->recv_mutex);
    res = _core_http_recv(http_handle, (uint8_t *)buffer, buffer_len, http_handle->recv_timeout_ms * 10);
...
int32_t core_http_recv(void *handle)
{
    ...
    res = _core_http_recv_body(http_handle, body_total_len - http_handle->session.body_read_len);
