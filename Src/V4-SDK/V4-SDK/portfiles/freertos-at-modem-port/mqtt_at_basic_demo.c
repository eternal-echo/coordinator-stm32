...
int32_t demo_mqtt_start(void **handle, char *product_key, char *device_name, char *device_secret)
{
    ...
    */
    ...
    g_mqtt_process_thread_running = 1;
		osThreadDef(processTask, demo_mqtt_process_thread, osPriorityHigh, 0, 460);
    ...
    g_mqtt_recv_thread_running = 1;
		osThreadDef(recvTask, demo_mqtt_recv_thread, osPriorityHigh, 0, 460);
...
void* log_init(void *mqtt_handle)
{
    ...
    int32_t res = aiot_logpost_setopt(logpost_handle, AIOT_LOGPOSTOPT_MQTT_HANDLE, mqtt_handle);
    if (res < STATE_SUCCESS) {
...
extern volatile uint64_t client_server_detal_time;
...
void demo_ntp_recv_handler(void *handle, const aiot_ntp_recv_t *packet, void *userdata)
{
    switch (packet->type) {
        ...
        case AIOT_NTPRECV_LOCAL_TIME: {
			...
			client_server_detal_time = packet->data.local_time.timestamp - g_aiot_sysdep_portfile.core_sysdep_time();
        }
        break;
...
void *ntp_start(void* mqtt_handle)
{
    int32_t     res = STATE_SUCCESS;
    int8_t      time_zone = 8;
...
int link_main(int argc, char *argv[])
{
    ...
    /* 创建1个logpost客户端实例并内部初始化默认参数 */
    logpost_handle = log_init(mqtt_handle);
    if (logpost_handle == NULL) {
        printf("aiot_logpost_init failed\r\n");
        return -1;
    }
