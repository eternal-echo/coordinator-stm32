/**
 * @file linux_tcp_modem_port.c
 * @brief 提供了基于Linux/MacOS+外部通信模组形态的SDK系统对接层实现
 * @date 2020-8-19
 *
...
{
    int remain =  xPortGetFreeHeapSize();
    void * result = pvPortMalloc(size);
    if(NULL == result) {
        printf("failed to malloc: remain mem is %u, now malloc %u\n",remain, size);
    }
    return result;
}
...
{
    return (uint64_t)(xTaskGetTickCount() * portTICK_RATE_MS + client_server_detal_time);
